import java.util.*;
import java.util.Collections;
PFont font;

int cols = 11;//列
int rows = 11;//行
int cellSize = 75;//行間距離

//PVector[][] prev = new PVector[cols][rows];//座標
//boolean[][] nodechack = new boolean[cols][rows];//ノードの有無

//int[][] dist = new int[cols][rows];//探索距離

void setup() {
  background(255);
  fullScreen();

  // 日本語を含むフォントを指定
  font = createFont("MS Gothic", 16);
  textFont(font);

  view();
}

void draw() {
}

void set() {
  //setRowDistances();//探索距離の設定
  reset();
  NodeLimitSet();
  NodeDataSet();
  lineLimitSet();
  generateRandomConnections();
  
  loadSkillsFromCSV("SampleData_Skill.csv");//追加部分
  loadStatusFromCSV("SampleData_Status.csv");//追加部分
}

void reset() {
  nodeSum = 0;
  nodeData.clear();
  lineData.clear();
}

void view() {
  int maxRetry = 1000;
  int retry = 0;

  do {
    set();
    retry++;
    if (retry > maxRetry) {
      println("警告: 入力0ノードが消せませんでした");
      break;
    }
  } while (hasNodeWithZeroInput());  // ← 入力ゼロがあれば再生成

  fill(255);
  stroke(0);
  rect(50, 50, 100, 60);
  fill(0);
  text(retry, 100, 100);

  drawGrid();//グリッドの表示
  lineView();
  nodeView();
  NodeCheck();
}

void keyPressed() {
  if (key == 'r' || key == 'R') {
    view();
  }

  if (key == 'd' || key == 'D') {
    redraw();
  }
  
  if(key == 'f' || key == 'F'){
    buildGraph();
    findAllShortestPaths();
  }
  
  if(key == 'a' || key == 'A'){
     println("shortestPaths.size() = " + shortestPaths.size());  // ← ここで確認
     if (shortestPaths.isEmpty()) {
      println("最短経路が未計算です。先に 'f' キーで findAllShortestPaths() を実行してください。");
    } else {
      SkillSetter();  
    }
  }
}

boolean hasNodeWithZeroInput() {
  for (Node n : lineData) {
    if (n.getId()== 0) continue;
    if (n.getInputCount() == 0) {
      return true;
    }
  }
  return false;
}
