...
void NodeLimitData() {//各階層でノードの個数の制限
  ...
  nodelimitPerRow.put(2, 6);
  nodelimitPerRow.put(3, 10);
  nodelimitPerRow.put(4, 7);
  nodelimitPerRow.put(5, 5);
  nodelimitPerRow.put(6, 2);
  nodelimitPerRow.put(7, 1);
}

void lineLimitData() {//階層のノード数に対して枝数の確率(最高枝数６)
  // 階層のノード数が2個のとき → 枝1: 50%, 枝2: 30%, 枝3: 20%
  linelimitPerRow.put(0, new float[]{0, 0, 0, 0, 0, 0, 0});
  linelimitPerRow.put(1, new float[]{0, 1, 0, 0, 0, 0.000, 0.00});
  linelimitPerRow.put(2, new float[]{0, 0.25, 0.25, 0, 0.5, 0, 0});
  linelimitPerRow.put(3, new float[]{0, 0, 0, 0, 1, 0, 0});
  linelimitPerRow.put(4, new float[]{0, 0, 0, 0, 0, 0.000, 0.000});
  linelimitPerRow.put(5, new float[]{0, 0, 0.4, 0, 0.4, 0.2, 0});
  linelimitPerRow.put(6, new float[]{0, 0, 0, 0, 0.66, 0, 0.34});
  linelimitPerRow.put(7, new float[]{0, 0, 0.14, 0.29, 0.14, 0.43, 0.000});
  linelimitPerRow.put(8, new float[]{0, 0, 0, 0, 0, 0, 0});
  linelimitPerRow.put(9, new float[]{0, 0, 0, 0, 0, 0, 0});
  linelimitPerRow.put(10, new float[]{0, 0, 0.1, 0.2, 0.5, 0.2, 0});
}
